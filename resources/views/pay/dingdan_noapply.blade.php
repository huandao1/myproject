...
<div class="main">
    <div style="width: 7rem;height: 7.03rem;margin:0 auto;margin-top: 0.5rem;z-index: 1">
        <img src="{{URL::asset('assets/image/zhifu@3x2.png')}}" alt="" style='width: 7rem;height: 7.03rem;' onclick="return false;">
    </div>
    <div style="width: 7rem;height: 7.03rem;margin:0 auto;margin-top: -7.03rem;z-index: 2;position: relative;">
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:0.86rem;">泊位编号</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:0.86rem;">{{$resdata['Data']['BerthCode']}}</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:1.70rem;">路段名称</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:1.70rem;">{{$resdata['Data']['SectionName']}}</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:2.58rem;">开始时间</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:2.58rem;">{{$resdata['Data']['StartParkingTime']}}</p>
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:3.56rem;">停车时长</p>
        <p style=";padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:3.56rem;">
        @php
            $er = $resdata['Data']['ParkDuration'];
        if ($er >= 60) {
           echo intval($er / 60) . "小时";
           $t = $er % 60;
           echo $t . "分钟";
       } elseif ($er > 0 && $er < 60) {
            echo $er . "分钟";
        ...
        <p style="width: 100%;padding-left: 1rem;font-size: 12px;color: #0d57a0;position: absolute;left: 1.4rem;top:4.47rem;"><span id="coupon_tips">离场超过72小时不可用</span></p>
        @elseif($coupon_use_array['Data']['UseStatus']==1)  {?>

        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:4.49rem;">停车优惠券</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 12px;color: #0d57a0;position: absolute;left: 1.4rem;top:4.47rem;"><span id="coupon_tips">不可重复使用优惠券</span></p>

        @else
        <p style="padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:4.49rem;">停车优惠券</p>
        <p style="padding-left: 1rem;font-size: 12px;color: #0d57a0;position: absolute;left: 1.4rem;top:4.47rem;" @if($i>0) onclick="show_coupon()"><span id="coupon_tips">{{$i}}张可用</span></p>
        @endif

        <p style="padding-left: .8rem;font-size: 0.28rem;color: #ffffff;position: absolute;left: 0;top:6.13rem;">停车计费</p>
        <p style="padding-left: 2.2rem;font-size: 0.46rem;color: red;position: absolute;left: 0;top:6.13rem;"><span class="jiaofei">￥{{$price}}</span></p>
        <input type="hidden" name="pay_monney0" class="pay_monney0" value="{{$resdata['Data']['ArrearsPrice']}}">
        ...
        <div class="coupon_true" style="margin-bottom: 60px">
            ...
            <div class ="opt">
                <div class="squaredThree">
                    <input type="radio" name="coupon_id" id="radio{{$t['VPNMId']}}" value="{{$t['VPNMId']}}-{{$t['VPMoney']}}-{{$t['VoucherType']}}" class="check">
                    <label for="myCheck"></label>
                </div>
                <div class="coupon_img0" onclick="change_coupon_select_status({{$t['VPNMId']}})">
                    <p class="coupon_top"><span class="coupon_size" >· </span>@if($t['AppeType']==1)满减券@else满减券@endif
                    </p>
                    <p class="coupon_left">使用期限：{{$t['EffTime']}}-{{$t['FailureTime']}}</p>
                    <p class="coupon_right"><span>@if($t['VoucherType']==1)小时@else￥@endif</span>{{$t['VPMoney']}}</p>
                            ...
                            <div class="opt">
                                ...
                                <div class="coupon_img0"
                                    ...
                                    <p class="coupon_top"><span
                                                class="coupon_size">· </span>@if($v['AppeType'] == 1)优惠券@else优惠券@endif
                                    </p>
...
<script>
    function fukuan(){
        var BargainOrder="@php echo isset($resdata['Data']['ArrearsOrderCode'])?$resdata['Data']['ArrearsOrderCode']:0;@endphp";
        var ordercode="{{$ordercode}}";
        var PayPrice="@php isset($resdata['Data']['ArrearsPrice'])?$resdata['Data']['ArrearsPrice']:0;@endphp";
        var berthcode="@php echo isset($resdata['Data']['BerthCode'])?$resdata['Data']['BerthCode']:0;@endphp";
        var time="@php echo isset($resdata['Data']['RemainTime'])?round($resdata['Data']['RemainTime']/60):0;@endphp";
        ...
        if(parseFloat(PayPrice)>0){
            window.location.href='pay_zhifu_noapply?ordercode='+ordercode+'&PayPrice='+PayPrice+'&berthcode='+berthcode+'&time='+time+'&BargainOrder='+BargainOrder+ "&VPNMId=" + VPNMId +"&VoucherID=" + VoucherID + "&CouponAmount=" + CouponAmount + "&CouponType=" + CouponType;
        }else{
            console.log(ops)
        }
