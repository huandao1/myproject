...
<div class="main">
    ...
    <div style="width: 7rem;height: 7.03rem;margin:0 auto;margin-top: -7.03rem;z-index: 2;position: relative;">
        ...
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:0.86rem;">{{$berthcode}}</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:1.70rem;">路段名称</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:1.70rem;">{{$sectionname}}</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:2.58rem;">停车时间</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:2.58rem;">{{$star_times}}</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:3.56rem;">停车时长</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #585858;position: absolute;left: 1.4rem;top:3.56rem;">{{GetRTime($time*60)}}</p>
        @if($coupon_use_array['Data']['UseStatus']==1||$getine>259200)
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:4.49rem;">停车优惠券</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #0d57a0;position: absolute;left: 1.4rem;top:4.49rem;"><span id="coupon_tips">0张可用</span></p>
        @else
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #cacaca;position: absolute;left: 0;top:4.49rem;">停车优惠券</p>
        <p style="width: 100%;padding-left: 1rem;font-size: 0.26rem;color: #0d57a0;position: absolute;left: 1.4rem;top:4.49rem;" @if($coupon_list['Data']['Count']>0)onclick="show_coupon()"><span id="coupon_tips">{{$i}}张可用</span></p>
        @endif
        ...
        <input type="hidden" name="pay_monney0" class="pay_monney0" value="{{$price}}">
                        ...
                        <div class="opt">
                            <div class="squaredThree">
                                <input type="radio" name="coupon_id" id="radio{{$t['VPNMId']}}"
                                       value="{{$t['VPNMId']}}-{{$t['VPMoney']}}-{{$t['VoucherType']}}"
                                       class="check">
                                <label for="myCheck"></label>
                            </div>
                            <div class="coupon_img0" onclick="change_coupon_select_status({{$t['VPNMId']}})">
                                <p class="coupon_top"><span
                                            class="coupon_size">· </span>@if($t['AppeType'] == 1)广发银行CGB@else优惠券@endif
                                </p>
                                <p class="coupon_left">使用期限：{{$t['EffTime']}}
                                    -{{$t['FailureTime']}}</p>
                                <p class="coupon_right">
                                    <span>@if($t['VoucherType'] == 1)小时@else￥</span>{{$t['VPMoney']}}@endif
                                </p>
                        ...
                        @if(in_array($v['PaymentType'], $py))
                            <div class="opt">
                                ...
                                <div class="coupon_img0"
                                    ...
                                    <img src="{{URL::asset('assets/image/bank'.$v['AppeType'].'png')}}"
                                         class="apptype"/>
                                </div>
                            </div>
                        @endif
...
<script>
    function fukuan(){
        ...
        var time="@php isset($time)?round($time/60):0;@endphp";
        ...
        $.ajax({
            ...
            success: function (ops) {
                ...
                if(ops.Message=='success'&&ops.Data.code=='finished'){
                    //alert(ops.Data.code);
                    //alert(ops.Data.data.ArrearsOrderCode);
